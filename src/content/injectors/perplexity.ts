...
export const injectPerplexity = async (pending: PendingHandoff) => {
  let mainFile: File;
  ...
  const interval = setInterval(() => {
    attempts++;
    const textarea = document.querySelector('textarea[placeholder*="Ask"], textarea') as HTMLTextAreaElement;
    ...
    if (textarea && fileInput) {
      ...
      if (settings.autoSend) {
        setTimeout(() => {
          const sendBtn = document.querySelector('button[aria-label*="Submit"], button[class*="submit"], button[class*="send"]') as HTMLButtonElement;
    ...
    if (attempts >= maxAttempts) {
      clearInterval(interval);
      console.warn('[MoveChat] Perplexity inputs not found.');
    }
  }, 1000);
};
