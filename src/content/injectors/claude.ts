...
export const injectClaude = async (pending: PendingHandoff) => {
  let mainFile: File;
  ...
  const interval = setInterval(() => {
    attempts++;
    const editor = document.querySelector('div[contenteditable="true"], div.ProseMirror') as HTMLDivElement;
    ...
    if (editor && fileInput) {
      ...
      if (settings.autoSend) {
        setTimeout(() => {
          const sendBtn = document.querySelector('button[aria-label*="Send"], button[class*="send"], button[class*="Submit"]') as HTMLButtonElement;
    ...
    if (attempts >= maxAttempts) {
      clearInterval(interval);
      console.warn('[MoveChat] Claude inputs not found.');
    }
  }, 1000);
};
